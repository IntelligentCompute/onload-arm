...
static void *find_syscall_table_kprobe(void)
{
#ifdef CONFIG_KPROBES
  /* kprobes can only attach to functions (text), not data symbols like
   * sys_call_table. So we use kprobes to resolve kallsyms_lookup_name
   * (a function), then call it to find sys_call_table (data). */
  ...
  if (register_kprobe(&kp) < 0) {
    TRAMP_DEBUG("%s: kprobe lookup of kallsyms_lookup_name failed", __FUNCTION__);
    return NULL;
  }
  fn = (kallsyms_lookup_name_t)kp.addr;
  unregister_kprobe(&kp);

  addr = (void *)fn("sys_call_table");
  if (addr == 0) {
    TRAMP_DEBUG("%s: kallsyms_lookup_name(sys_call_table) returned NULL", __FUNCTION__);
    return NULL;
  }
  TRAMP_DEBUG("%s: Found sys_call_table via kprobe+kallsyms at %px", __FUNCTION__, addr);
  return addr;
#else
  return NULL;
#endif
...
static void *find_syscall_table(void)
{
  ...
  /* Method 1: Try kprobe-based lookup (works on any kernel with CONFIG_KPROBES) */
  syscalls = find_syscall_table_kprobe();
  if (syscalls != NULL)
    return syscalls;

  /* Method 2: Try module parameter (userspace can provide address via /proc/kallsyms) */
  syscalls = find_syscall_table_module_param();
  if (syscalls != NULL)
    return syscalls;

  /* Method 3: Try efrm_find_ksym approach (works when kallsyms_on_each_symbol is exported) */
